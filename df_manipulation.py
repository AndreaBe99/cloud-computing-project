...
def create_main_cols(all_season, x, team):
    ...
    total_points, total_l_points, total_l_w_avg_points, total_goals, total_l_goals, total_l_w_avg_goals, total_goals_sf, total_l_goals_sf, total_l_w_avg_goals_sf, total_wins, total_draws, total_losses, win_streak, loss_streak, draw_streak = get_match_stats(
        all_season, x)
...
def get_rank(all_season, x, team, delta_year):
    ...
    full_home_df = full_season_df.groupby(['HomeTeam']).sum(
    )[['h_match_points', 'FTHG', 'FTAG']].reset_index()
    full_home_df.columns = ['team', 'points', 'goals', 'goals_sf']

    full_away_df = full_season_df.groupby(['AwayTeam']).sum(
    ...
    rank_df = rank_df.sort_values(
        by=['points', 'goals_df', 'goals'], ascending=False)
    rank_df['rank'] = rank_df.points.rank(
